...
import { systemActivity } from '../../data/adminData';
...
const Activity = () => {
  ...
  const filteredActivities = useMemo(() => {
    return systemActivity.filter(item => {
      const matchesType = filterType === 'all' || item.type === filterType;
      const matchesDate = !filterDate || item.timestamp.startsWith(filterDate);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[14px] text-text-muted mt-2 font-light max-w-2xl leading-relaxed">
            A comprehensive timeline of all events across the GLACIER ecosystem. Monitor transactions, user interactions, and operational alerts.
          </p>
        </div>
