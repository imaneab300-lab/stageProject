      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20 md:h-24">
          ...
          <Link to="/" className="flex-shrink-0 group">
            <span className="font-serif italic text-[24px] md:text-[28px] text-cyan-500 tracking-[0.15em] font-semibold transition-all group-hover:tracking-[0.2em]">
              GLACIER
            </span>
          </Link>
                        ...
                        .map(p => (
                          ...
                          >
                            <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-aether-800 border border-glass-border shadow-sm p-2">
                              <img src={p.images?.[0] || p.image} alt={p.name} className="w-full h-full object-contain grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-110" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-[13px] font-semibold text-text-primary truncate uppercase tracking-tight">{p.name}</p>
                              <p className="text-[11px] text-text-muted uppercase tracking-[0.1em] font-medium mt-1">{p.category}</p>
                            </div>
                            <span className="text-[14px] font-serif font-semibold text-cyan-500 tracking-tight">${p.price.toLocaleString()}</span>
                          </Link>
                        ))
                      }
                    ...
                    <div className="max-h-80 overflow-y-auto divide-y divide-glass-border custom-scrollbar">
                      {!user ? (
                        ...
                        notifications.map((n) => (
                          <div
                            key={n.id}
                            className={`px-5 py-4 flex gap-4 items-start transition-all hover:bg-aether-600 ${!n.read ? 'bg-cyan-500/5' : ''}`}
                          >
