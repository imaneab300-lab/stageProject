...
import { adminOrders, adminUsers } from '../../data/adminData';
...
const AdminTopbar = ({ pageTitle }) => {
  ...
  const handleSearch = (query) => {
    ...
    const filteredOrders = adminOrders
      ...
      .map(o => ({ ...o, name: o.id, subtext: o.customer, type: 'order', icon: <Receipt className="w-3.5 h-3.5" />, link: `/admin/orders` }));

    const filteredUsers = adminUsers
      .filter(u => u.name.toLowerCase().includes(query.toLowerCase()) || u.email.toLowerCase().includes(query.toLowerCase()))
      .map(u => ({ ...u, subtext: u.email, type: 'user', icon: <UserIcon className="w-3.5 h-3.5" />, link: `/admin/users` }));

    setSearchResults([...filteredProducts, ...filteredOrders, ...filteredUsers].slice(0, 8));
    setSearchOpen(true);
  };
              ...
              {profileOpen && (
                ...
                >
                  <div className="p-4 border-b border-glass-border bg-aether-800/30">
                    <p className="text-[10px] font-bold text-text-primary uppercase tracking-widest truncate">{user?.name || 'Glacier Admin'}</p>
                    <p className="text-[8px] text-text-muted font-bold uppercase tracking-tighter truncate opacity-60">{user?.email || 'admin@glacier.luxe'}</p>
                  </div>
